...
model_file =('model/model_lr_v1_%d' %axis)
	...
	for b in range(batch):
		for s in range(SEQ):
			for i in range(input_size):
				#print(data[ptr+b+s][i])
				#print(data[b+ptr+s+interval])
		...
		temp = (math.log((float)(data[b+ptr+SEQ+1][axis]))-math.log((float)(data[b+ptr+SEQ+1+interval][axis])))/math.log(2)
		if ((float)(temp) <= 0):
			y[b] = [1, 0]
		else:
			y[b] = [0, 1]
...
if __name__ == '__main__':
	...
	print("Training ...")
	ftrain = open('result/train_lr_hour_v1_%d.txt' %axis, 'wb')
	fval = open('result/val_lr_hour_v1_%d.txt' %axis, 'wb')
	ftest = open('result/test_lr_hour_v1_%d.txt' %axis, 'wb')
